...
<div class="main-container">
  ...
  <!-- EQ PANEL -->
  <div class="dsp-panel">
    <div class="dsp-panel-title"><span class="status-dot"></span>Ecualizador Paramétrico 6 Bandas</div>
  ...
  <!-- AGC PANEL -->
  <div class="dsp-panel">
    <div class="dsp-panel-title"><span class="status-dot"></span>AGC - Automatic Gain Control</div>
  ...
  <div class="dsp-panel">
    <div class="dsp-panel-title"><span class="status-dot"></span>Multiband Compressor (4 Bandas)</div>
    <div id="multiband4Knobs"></div>
  </div>

  <!-- 3-BAND COMPRESSOR -->
  <div class="dsp-panel">
    <div class="dsp-panel-title"><span class="status-dot"></span>Compresor 3 Bandas</div>
    <div id="comp3Knobs"></div>
  </div>

  <!-- BAND LIMITER 3 BANDAS -->
  <div class="dsp-panel">
    <div class="dsp-panel-title"><span class="status-dot"></span>Band Limiter 3 Bandas</div>
    <div id="limiter3Knobs"></div>
  </div>

  <!-- FINAL LIMITER -->
  <div class="dsp-panel">
    <div class="dsp-panel-title"><span class="status-dot"></span>Limiter Final</div>
  ...
  <!-- POWER LIMITER -->
  <div class="dsp-panel">
    <div class="dsp-panel-title"><span class="status-dot"></span>Power Limiter</div>
  ...
  <div class="dsp-row">
    <div class="dsp-panel">
      <div class="dsp-panel-title"><span class="status-dot"></span>Pre-Emphasis</div>
      <div class="knobs-grid">
        <div class="knob-container">
...
class BroadcastProcessor {
  ...
  initDefaultParams() {
    ...
    // Pre-Emphasis
    this.params.preEmphasis = 0; // 0=50us, 1=75us
    this.params.preEmphasisGain = 0;

    // Output
    this.params.outputGain = 0;
    this.params.outputLimit = 100;
    this.params.stereoMode = 0; // 0=stereo, 1=mono, 2=mpx
  ...
  async initAudio() {
    try {
      ...
      // Get input
      // Request audio stream using selected input device if set
      const constraints = {
        audio: {
      ...
      // Output device handling (Sink ID)
  ...
  generateWorkletCode() {
    ...
    class BroadcastProcessor extends AudioWorkletProcessor {
      ...
      createFilter(type, freq, order) {
        // Simple 1st order IIR for crossovers
      ...
      processPreEmphasis(x) {
        ...
        // Simple first-order pre-emphasis (high shelf boost)
      ...
      process(inputs, outputs, parameters) {
        ...
        for (let i = 0; i < blockSize; i++) {
          ...
          // 4. Multiband Compressor (4 bands)
          ...
          // Pre-emphasis
          if (this.preEmphasisGain > 0) {
            sL = this.processPreEmphasis(sL);
            sR = this.processPreEmphasis(sR);
          }
          
          // Stereo processing
          if (this.stereoMode === 1) { // Mono
            const mid = (sL + sR) / 2;
            sL = mid;
            sR = mid;
          } else if (this.stereoMode === 2) { // MPX-like (enhanced stereo)
            const mid = (sL + sR) / 2;
            const side = (sL - sR) / 2;
      ...
      computeSpectrum(buffer) {
        ...
        // Simple Goertzel-like energy per band
        const bands = 128;
  ...
  updateMeters() {
    ...
    // Processed meters (simulated from output)
  ...
  drawSpectrum() {
    ...
    for (let i = 0; i < this.spectrumData.length; i++) {
      ...
      if (intensity < 0.5) {
        ...
        r = 255;
        g = Math.floor((1 - (intensity - 0.8) * 5) * 255);
        b = 0;
      }
  ...
  initUI() {
    ...
    document.getElementById('outputDevice').addEventListener('change', async (e) => {
      ...
      if (this.audioCtx && this.audioCtx.setSinkId) {
        try {
          await this.audioCtx.setSinkId(e.target.value);
        } catch (err) {
  ...
  initStandardKnobs() {
    document.querySelectorAll('.knob-wrapper').forEach(wrapper => {
      ...
      const label = wrapper.parentElement.querySelector('.knob-label');
      ...
      const updateValue = (val) => {
        ...
        let displayValue;
        if (param.includes('Gain') || param.includes('gain') || param.includes('Gain') || param.includes('threshold') || param.includes('Threshold')) {
        ...
        // Send to DSP
        if (param === 'eq') return; // handled separately
        if (param.startsWith('eq_')) return; // handled separately
        this.sendParam(param, currentValue);
      };
  ...
  initEQKnobs() {
    ...
    container.querySelectorAll('.knob-wrapper').forEach(wrapper => {
      ...
      window.addEventListener('mousemove', (e) => {
        if (!isDragging) return;
        updateValue(startValue + (startY - e.clientY) * ((max - min) / 200));
      });
      window.addEventListener('mouseup', () => { isDragging = false; });
      wrapper.addEventListener('wheel', (e) => {
        e.preventDefault();
        updateValue(currentValue - e.deltaY * ((max - min) / 1000));
      }, { passive: false });
    ...
    container.querySelectorAll('[data-eqtype]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        btn.classList.add('active');
        btn.closest('.band-section').querySelector('.band-title').textContent = 
          `Band ${bandIdx + 1} - ${this.params.eq[bandIdx].type}`;
  ...
  initAGCKnobs() {
    ...
    this.bindKnobGroup('agc_threshold', 'threshold', agc.threshold, (v) => {
      this.params.agc.threshold = v;
      this.audioWorklet?.port.postMessage({ type: 'agc', params: this.params.agc });
    });
    this.bindKnobGroup('agc_ratio', 'ratio', agc.ratio, (v) => {
      this.params.agc.ratio = v;
      this.audioWorklet?.port.postMessage({ type: 'agc', params: this.params.agc });
    });
    this.bindKnobGroup('agc_attack', 'attack', agc.attack, (v) => {
      this.params.agc.attack = v;
      this.audioWorklet?.port.postMessage({ type: 'agc', params: this.params.agc });
    });
    this.bindKnobGroup('agc_release', 'release', agc.release, (v) => {
      this.params.agc.release = v;
      this.audioWorklet?.port.postMessage({ type: 'agc', params: this.params.agc });
    });
  }
  ...
  initMultiband4Knobs() {
    ...
    // Bind knobs
    const paramMap = {
      'mb4_freq': 'freq', 'mb4_thr': 'threshold', 'mb4_rat': 'ratio',
      'mb4_atk': 'attack', 'mb4_rel': 'release', 'mb4_gn': 'gain'
    };
    const ranges = {
      'mb4_freq': [40, 16000], 'mb4_thr': [-60, 0], 'mb4_rat': [1, 20],
      'mb4_atk': [0.1, 200], 'mb4_rel': [10, 2000], 'mb4_gn': [-12, 12]
    };
    
    this.params.multiband4.forEach((band, i) => {
      Object.entries(paramMap).forEach(([prefix, param]) => {
        this.bindKnobGroup(`${prefix}_${i}`, param, band[param], (v) => {
          this.params.multiband4[i][param] = v;
          this.sendMultiband4(i, { [param]: v });
        });
      });
    });
  ...
  initComp3Knobs() {
    ...
    const paramMap = {
      'c3_freq': 'freq', 'c3_thr': 'threshold', 'c3_rat': 'ratio',
      'c3_atk': 'attack', 'c3_rel': 'release', 'c3_gn': 'gain'
    };
    
    this.params.comp3.forEach((band, i) => {
      Object.entries(paramMap).forEach(([prefix, param]) => {
        this.bindKnobGroup(`${prefix}_${i}`, param, band[param], (v) => {
          this.params.comp3[i][param] = v;
          this.sendComp3(i, { [param]: v });
        });
      });
    });
  ...
  initLimiter3Knobs() {
    ...
    const paramMap = {
      'l3_freq': 'freq', 'l3_thr': 'threshold', 'l3_rat': 'ratio',
      'l3_atk': 'attack', 'l3_rel': 'release', 'l3_gn': 'gain'
    };
    
    this.params.limiter3.forEach((band, i) => {
      Object.entries(paramMap).forEach(([prefix, param]) => {
        this.bindKnobGroup(`${prefix}_${i}`, param, band[param], (v) => {
          this.params.limiter3[i][param] = v;
          this.sendLimiter3(i, { [param]: v });
        });
      });
    });
  ...
  initFinalLimiterKnobs() {
    ...
    this.bindKnobGroup('fl_thr', 'threshold', fl.threshold, (v) => {
      this.params.finalLimiter.threshold = v;
      this.sendFinalLimiter({ threshold: v });
    });
    this.bindKnobGroup('fl_rat', 'ratio', fl.ratio, (v) => {
      this.params.finalLimiter.ratio = v;
      this.sendFinalLimiter({ ratio: v });
    });
    this.bindKnobGroup('fl_atk', 'attack', fl.attack, (v) => {
      this.params.finalLimiter.attack = v;
      this.sendFinalLimiter({ attack: v });
    });
    this.bindKnobGroup('fl_rel', 'release', fl.release, (v) => {
      this.params.finalLimiter.release = v;
      this.sendFinalLimiter({ release: v });
    });
    this.bindKnobGroup('fl_gn', 'gain', fl.gain, (v) => {
      this.params.finalLimiter.gain = v;
      this.sendFinalLimiter({ gain: v });
    });
  }
  ...
  initPowerLimiterKnobs() {
    ...
    this.bindKnobGroup('pl_thr', 'threshold', pl.threshold, (v) => {
      this.params.powerLimiter.threshold = v;
      this.sendPowerLimiter({ threshold: v });
    });
    this.bindKnobGroup('pl_rat', 'ratio', pl.ratio, (v) => {
      this.params.powerLimiter.ratio = v;
      this.sendPowerLimiter({ ratio: v });
    });
    this.bindKnobGroup('pl_atk', 'attack', pl.attack, (v) => {
      this.params.powerLimiter.attack = v;
      this.sendPowerLimiter({ attack: v });
    });
    this.bindKnobGroup('pl_rel', 'release', pl.release, (v) => {
      this.params.powerLimiter.release = v;
      this.sendPowerLimiter({ release: v });
    });
    this.bindKnobGroup('pl_gn', 'gain', pl.gain, (v) => {
      this.params.powerLimiter.gain = v;
      this.sendPowerLimiter({ gain: v });
    });
  }
  ...
  loadPreset(name) {
    ...
    // Apply input/output
    if (preset.inputGain !== undefined) {
      this.sendParam('inputGain', preset.inputGain);
      this.params.inputGain = preset.inputGain;
    }
    if (preset.outputGain !== undefined) {
      this.sendParam('outputGain', preset.outputGain);
      this.params.outputGain = preset.outputGain;
    }
...
document.getElementById('latencyDisplay').textContent = 
  (processor.bufferSize / 48000 * 1000).toFixed(2) + ' ms';

// Animate frame counter for CPU estimation
